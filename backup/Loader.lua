task.spawn(function() -- loader
	_G["this is only the loader random dude, sanity is coming.."] = true;
	...
	local LoadTween = TweenService:Create(ProgressBar, TweenInfo.new(5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
	...
	local function CloseGui()
		...
		TweenService:Create(TitleLabel, FadeOutInfo,
			{
				TextTransparency = 1,
		...
		TweenService:Create(BarBackground, FadeOutInfo,
			{ BackgroundTransparency = 1, Position = UDim2.new(0.5, -200, 0.8, 0) }):Play()
		TweenService:Create(ProgressBar, FadeOutInfo, { BackgroundTransparency = 1 }):Play()
		TweenService:Create(SkipButton, FadeOutInfo,
			{ BackgroundTransparency = 1, TextTransparency = 1, Position = UDim2.new(0.5, -60, 0.9, 0) }):Play()
	...
	TweenService:Create(TitleLabel, FadeInInfo,
		{
			TextTransparency = 0,
	...
	TweenService:Create(BarBackground, FadeInInfo,
		{ BackgroundTransparency = 0, Position = UDim2.new(0.5, -200, 0.75, 0) }):Play()
	TweenService:Create(ProgressBar, FadeInInfo, { BackgroundTransparency = 0 }):Play()
	TweenService:Create(SkipButton, FadeInInfo,
		{ BackgroundTransparency = 0, TextTransparency = 0, Position = UDim2.new(0.5, -70, 0.83, 0) }):Play()
	...
	SkipButton.MouseEnter:Connect(function()
		TweenService:Create(SkipButton, TweenInfo.new(0.2),
			{ Size = UDim2.new(0, 150, 0, 48), Position = UDim2.new(0.5, -75, 0.828, 0) }):Play()
		TweenService:Create(ButtonStroke, TweenInfo.new(0.2), { Thickness = 3 }):Play()
	end)

	SkipButton.MouseLeave:Connect(function()
		TweenService:Create(SkipButton, TweenInfo.new(0.2),
			{ Size = UDim2.new(0, 140, 0, 45), Position = UDim2.new(0.5, -70, 0.83, 0) }):Play()
	...
	SkipButton.MouseButton1Down:Connect(function()
		TweenService:Create(SkipButton, TweenInfo.new(0.1),
			{ Size = UDim2.new(0, 135, 0, 42), Position = UDim2.new(0.5, -67.5, 0.832, 0) }):Play()
	end)
